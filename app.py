...
if uploaded_file:
    ...
    # Colunas auxiliares - simplificando a abordagem
    df["AnoMes"] = df["Abertura"].dt.strftime('%Y-%m')  # Formato que ordena corretamente
    df["AnoMes_Display"] = df["Abertura"].dt.strftime('%b/%Y')  # Formato de exibição
    ...
    if df_filtrado.empty:
        ...
        # Criar ordenação cronológica definitiva
        meses_ordenados = sorted(df_filtrado["AnoMes"].unique())
        meses_display_ordenados = [pd.to_datetime(m).strftime('%b/%Y') for m in meses_ordenados]

        ## 1️⃣ Total de Casos por Mês
        st.subheader("1️⃣ Total de Casos por Mês")
        casos_mes = df_filtrado.groupby("AnoMes")["AnoMes_Display"].count().reset_index(name="Total")
        ...
        fig1 = px.bar(
            ...
            category_orders={"AnoMes_Display": meses_display_ordenados}
        )
        fig1.update_traces(textposition='outside')
        fig1.update_xaxes(type='category', categoryorder='array', categoryarray=meses_display_ordenados)
        ...
        fig2.update_xaxes(type='category', categoryorder='array', categoryarray=meses_display_ordenados)
        ...
        fig3.update_xaxes(type='category', categoryorder='array', categoryarray=meses_display_ordenados)
        ...
        fig5.update_xaxes(type='category', categoryorder='array', categoryarray=meses_display_ordenados)
